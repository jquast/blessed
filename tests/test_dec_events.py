...
def bracketed_paste_sequence():
    """Fixture for bracketed paste test sequence."""
    return '\x1b[200~hello world\x1b[201~'


@pytest.fixture 
def multiline_paste_sequence():
    """Fixture for multiline paste test sequence."""
    return '\x1b[200~line1\nline2\tindented\x1b[201~'


@pytest.fixture
def sgr_mouse_sequences():
    """Fixture for SGR mouse test sequences."""
    return {
        'press': '\x1b[0;10;20M',  # Left button press at (10,20)
        'release': '\x1b[0;15;25m',  # Left button release at (15,25)
        'modifiers': '\x1b[28;5;5M',  # Button with shift+meta+ctrl (4+8+16=28)
...
def legacy_mouse_sequences():
    """Fixture for legacy mouse test sequences."""
    return {
        'press': '\x1b[M   ',  # Button 0 (32), pos (32,32) = (0,0) after -32
        'release': '\x1b[M#@@'  # Button 3 (35=32+3), release
    }


@pytest.fixture
def focus_sequences():
    """Fixture for focus event test sequences."""
    return {
        'gained': '\x1b[I',
        'lost': '\x1b[O'
    }
...
class TestDECEventMatching:
    ...
    def test_mouse_sgr_events(self, sgr_mouse_sequences):
        ...
        values = ks_press.get_event_values()
        ...
        values = ks_release.get_event_values()
        assert values.x == 15
        assert values.y == 25
        ...
        values = ks_mod.get_event_values()
...
def test_resolve_sequence():
    ...
    event_value = ks_dec.get_event_values()

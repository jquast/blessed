...
def rgb_at_xy(term, x, y, t):
    ...
    saturation = y / h
    lightness = x / w
    return tuple(map(scale_255, colorsys.hsv_to_rgb(
        hue / 8.0, saturation, lightness)))
...
def show_please_wait(term):
    ...
    outp = term.move(term.height - 1, 0) + \
        term.clear_eol + term.center(txt_wait)
    print(outp, end='')
    sys.stdout.flush()
...
def show_paused(term):
    txt_paused = 'paused'
    outp = term.move(term.height - 1,
                     int(term.width / 2 - len(txt_paused) / 2))
    outp += txt_paused
    print(outp, end='')
    sys.stdout.flush()
